package com.evnit.ttpm.AuthApp.service.category;

import com.evnit.ttpm.AuthApp.entity.admin.ResponseData;
import com.evnit.ttpm.AuthApp.model.category.LicenseOperate.LicenseOperateCreateDto;

public interface SLicenseOperateServcie {
	ResponseData getAll();
	ResponseData getById(int id);
	ResponseData create(LicenseOperateCreateDto createDto);
	ResponseData update(int id, LicenseOperateCreateDto updateDto);
	ResponseData delete(int id);
	ResponseData delete1(int id);
}
