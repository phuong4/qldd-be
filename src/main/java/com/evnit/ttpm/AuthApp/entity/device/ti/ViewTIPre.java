...
@Data
@Entity
@Table(name = "VIEWTIPRE")
@AllArgsConstructor
@NoArgsConstructor
public class ViewTIPre {
    @Id
    @Column(name = "ASSETID")
    ...
    @OneToMany(mappedBy = "viewTIPre", cascade = CascadeType.ALL, fetch = FetchType.LAZY)
    private List<ViewCuon> listCuon;
    @JsonIgnore
    @ToString.Exclude
    @ManyToOne
    @JoinColumn(name = "ASSETID_PARENT", insertable = false, updatable = false)
    private ViewTI setTI;
    @JsonIgnore
    @ToString.Exclude
    @ManyToOne
    @JoinColumn(name = "ASSETID_PARENT", insertable = false, updatable = false)
    private ViewSetTI viewSetTI;
